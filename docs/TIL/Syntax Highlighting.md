...
date: 06-06-2024
---
Today I added some syntax highlighting to my console logging functions. Figuring out the Regex was quite simple until I came across strings in json arrays... With some data looking like this:
```json
{
	"key1": [
		"value1",
...
```
It was rather hard to find a perfect match. I also can't "override" an existing coloring. So if I match anything twice the output would be broken.
In the end I managed to find a great working regex for strings in JSON: 
```regex
/("[^"]*"),?\r?\n/g
```
This targets all string values without recoloring the keys. Great. Next I just apply colors to these matches using chalk.
> [!Info]
> I only apply colors to the first matching group. I do this in order to be able to match characters I don't want to colorize. This leads to more accurate matches.
> ```typescript
